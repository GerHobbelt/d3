d3 = function() {
  ...
  var d3_timer_queueHead, d3_timer_queueTail, d3_timer_interval, d3_timer_timeout, d3_timer_active, d3_timer_frame = (typeof d3_window === "undefined" ? false : d3_window[d3_vendorSymbol(d3_window, "requestAnimationFrame")]) || function(callback) {
      ...
      if (g) {
        g.each(function() {
          ...
          var tick = g.selectAll(".tick.major").data(ticks, function(d, i) {
            return String(d.value);
          }), tickEnter = tick.enter().insert("g", ".domain").attr("class", "tick major").style("opacity", 1e-6), tickExit = d3.transition(tick.exit()).style("opacity", 1e-6).remove(), tickUpdate = d3.transition(tick).style("opacity", 1), tickTransform;
