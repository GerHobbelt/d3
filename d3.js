!function() {
    ...
    d3.behavior.zoom = function() {
        ...
        function touchstarted() {
            ...
            function started() {
                ...
                var touches = relocate(), now = Date.now(), p, q, dx, dy;
                if (touches.length === 1) {
        ...
        function pointerdowned() {
            ...
            function started() {
                ...
                if (pointers.length == 1) {
                    subject.on(d3_behavior_zoomTouch.down + ".zoom", null).on(d3_behavior_zoomTouch.down + zoomName, started).on(d3_behavior_zoomTouch.move + zoomName, moved).on(d3_behavior_zoomTouch.up + zoomName, ended).on(d3_behavior_zoomTouch.leave + zoomName, ended);
                }
                scale0 = view.k;
                if (pointers.length > 1) {
    ...
    function d3_locale_timeFormat(locale) {
        ...
        function d3_time_format(template) {
            ...
            format.parse = function(string) {
                ...
                var localZ = d.Z != null && d3_date !== d3_date_utc, date = localZ ? new d3_date_utc() : new d3_date();
    ...
    d3.geo.bounds = function() {
        ...
        function linePoint(λ, φ) {
            ...
            if (p0) {
                ...
                var dλ = λ - λ_, s = dλ > 0 ? 1 : -1, λi = inflection[0] * d3_degrees * s, antimeridian = abs(dλ) > 180, φi;
        ...
        return function(feature) {
            ...
            if (n) {
                ...
                for (n = merged.length - 1, i = 0, a = merged[n]; i <= n; a = b, ++i) {
                    b = merged[i];
    ...
    function d3_geo_clip(pointVisible, clipLine, interpolate, clipStart) {
        return function(rotate, listener) {
            ...
            function ringEnd() {
                ...
                var clean = ringListener.clean(), ringSegments = buffer.buffer(), segment, n = ringSegments.length, i, coord;
                ring.pop();
                polygon.push(ring);
                ring = null;
                if (!n) return;
    ...
    function d3_geo_clipCircle(radius) {
        ...
        function code(λ, φ) {
            var r = smallRadius ? radius : π - radius, bitvec = 0;
            if (λ < -r) bitvec |= 1; else if (λ > r) bitvec |= 2;
            if (φ < -r) bitvec |= 4; else if (φ > r) bitvec |= 8;
            return bitvec;
        }
    }
    ...
    d3.layout.chord = function() {
        ...
        function relayout() {
            var subgroups = {}, groupSums = [], groupIndex = d3.range(n), subgroupIndex = [], k, x, x0, i, j, di, dj, v, a0, a1;
            chords = [];
            groups = [];
            k = 0, i = -1;
            while (++i < n) {
    ...
    d3.layout.force = function() {
        var force = {}, event = d3.dispatch("start", "tick", "end"), timer, size = [ 1, 1 ], drag, alpha, interval, nodes = [], links = [], distances, strengths, epsilon = .1, charges, charge_abssum = -1, gravity_f, theta2_f, has_theta2_f = false, friction = .9, linkDistance = d3_layout_forceLinkDistance, linkStrength = d3_layout_forceLinkStrength, charge = -30, chargeDistance2 = d3_layout_forceChargeDistance2, gravity = .1, theta2 = .64;
        ...
        force.start = function() {
            ...
            function position(dimension, size, i) {
                ...
                var candidates = neighbors[i], l = candidates.length, x;
                for (j = 0; j < l; ++j) {
    ...
    d3.layout.histogram = function() {
        ...
        function histogram(data, index) {
            var bins = [], values = data.map(valuer, this), range = ranger.call(this, values, index), thresholds = binner.call(this, range, values, index), bin, i = -1, n = values.length, m = thresholds.length - 1, k = frequency ? 1 : 1 / n, x;
            while (++i < m) {
                bin = bins[i] = [];
    ...
    function d3_scale_linear(domain, range, interpolate, clamp) {
        ...
        scale.domainSegmented = function(x, n) {
            ...
            n -= 1;
            for (var i = 0; i <= n; ++i) domain.push(mn + d * i / n);
    ...
    function d3_scale_linearTickRange(domain, m) {
        if (m == null) m = 10;
        var extent = d3_scaleExtent(domain), span = extent[1] - extent[0], step, err;
    ...
    function d3_scale_log(linear, base, positive, domain) {
        ...
        scale.ticks = function() {
            var extent = d3_scaleExtent(domain), ticks = [], u = extent[0], v = extent[1], i = Math.floor(log(u)), j = Math.ceil(log(v)), n = base % 1 ? 2 : base, k;
            if (isFinite(j - i)) {
                if (positive) {
        ...
        scale.domainSegmented = function(x, n) {
            ...
            n -= 1;
            for (var i = 0; i <= n; ++i) domain.push(Math.pow(mn, (n - i) / n) * Math.pow(mx, i / n));
            return scale.domain(domain);
        };
    ...
    function d3_scale_pow(linear, exponent, domain) {
        ...
        scale.domainSegmented = function(x, n) {
            var mn = x[0], d = x[1] - mn, domain = [];
            n -= 1;
            for (var i = 0; i <= n; ++i) domain.push(mn + d * Math.pow(i / n, 1 / exponent));
            return scale.domain(domain);
        };
        scale.copy = function() {
    ...
    d3.svg.arc = function() {
        ...
        function arc() {
            ...
            var rc, cr, rp, ap, p0 = 0, p1 = 0, x0, y0, x1, y1, x2, y2, x3, y3, l0, l1, path = [];
    ...
    d3.svg.chord = function() {
        ...
        function subgroup(self, f, d, i) {
            var subgroup_f = f.call(self, d, i), r = radius.call(self, subgroup_f, i), a0 = startAngle.call(self, subgroup_f, i) - halfπ, a1 = endAngle.call(self, subgroup_f, i) - halfπ;
            return {
                r: r,
                a0: a0,
                a1: a1,
    ...
    d3_transitionPrototype.filter = function(filter) {
        var subgroups = [], subgroup, group, node, i, n, j, m;
        if (typeof filter !== "function") filter = d3_selection_filter(filter);
    ...
    d3_transitionPrototype.text = function(value) {
        ...
        var node = this.node(), tween = node.__transition__[this.id].tween.get("text");
    ...
    d3_transitionPrototype.transition = function() {
        var id0 = this.id, id1 = ++d3_transitionId, ns = this.namespace, subgroups = [], subgroup, group, node, transition, j, m, i, n;
        for (j = 0, m = this.length; j < m; j++) {
            subgroups.push(subgroup = []);
            for (group = this[j], i = 0, n = group.length; i < n; i++) {
                if (node = group[i]) {
