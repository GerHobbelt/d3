  ...
  function d3_geo_clipLineSegment(a, b) {
    ...
    function clipLine(listener) {
      ...
      return {
        ...
        point: function(λ, φ) {
          var δλ = λ - λ0, sλ = Math.abs(δλ) > π, λ1, φ1;
          if (δλ < 0) λ1 = λ0, λ0 = λ, φ1 = φ0, φ0 = φ; else λ1 = λ, φ1 = φ;
          var cosφ = Math.cos(φ), x = cosφ * Math.cos(λ), y = cosφ * Math.sin(λ), z = Math.sin(φ);
          if (next && (cλ0 <= λ1 || λ0 <= cλ1)) {
            var nx = y0 * z - z0 * y, ny = z0 * x - x0 * z, nz = x0 * y - y0 * x, m = Math.sqrt(nx * nx + ny * ny + nz * nz);
            nx /= m, ny /= m, nz /= m;
            var Nx = cny * nz - cnz * ny, Ny = cnz * nx - cnx * nz, Nz = cnx * ny - cny * nx;
            var λ2 = Math.atan2(Ny, Nx);
            if (csλ ^ (cλ0 <= λ2 && λ2 <= cλ1) && sλ ^ (λ0 <= λ2 && λ2 <= λ1) || (Nz = -Nz, 
            csλ ^ (cλ0 <= (λ2 = (λ2 + 2 * π) % (2 * π) - π) && λ2 <= cλ1) && sλ ^ (λ0 <= λ2 && λ2 <= λ1))) {
              var φ2 = Math.asin(Nz / Math.sqrt(Nx * Nx + Ny * Ny + Nz * Nz)), λε = δλ < 0 ? ε : -ε, φε = δλ < 0 ^ φ1 < φ0 ? -ε : ε;
    ...
    function interpolate(from, to, direction, listener) {
      if (from == null) {
        var λε = direction > 0 ^ δcλ < 0 ? ε : -ε, φε = direction > 0 ^ δcλ < 0 ^ cφ1 < cφ0 ? -ε : ε;
        listener.point(cλ0 - λε, cφ0 - φε);
        listener.point(cλ0 + λε, cφ0 + φε);
        listener.point(cλ1 + λε, cφ1 + φε);
        listener.point(cλ1 - λε, cφ1 - φε);
      } else {
        var cosφ, λ2 = from[0], φ2 = from[1], λ3 = to[0], φ3 = to[1], x2 = (cosφ = Math.cos(φ2)) * Math.cos(λ2), y2 = cosφ * Math.sin(λ2), z2 = Math.sin(φ2), x3 = (cosφ = Math.cos(φ3)) * Math.cos(λ3), y3 = cosφ * Math.sin(λ3), z3 = Math.sin(φ3), A = cnx * x2 + cny * y2 + cnz * z2 > 0;
        if (A ^ cnx * x3 + cny * y3 + cnz * z3 > 0) {
          if (A ^ direction > 0) listener.point(cλ1, cφ1); else listener.point(cλ0, cφ1);
        }
