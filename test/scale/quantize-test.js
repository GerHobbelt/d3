...
suite.addBatch({
  "quantize": {
    ...
    "clamps input values to the domain": function(quantize) {
      ...
      assert.equal(x(-0.5), a);
      assert.equal(x(1.5), c);
    },
    "range cardinality determines the degree of quantization": function(quantize) {
      var x = quantize();
      assert.inDelta(x.range(_.range(0, 1.001, 0.001))(1/3), 0.333, 1e-6);
      assert.inDelta(x.range(_.range(0, 1.01, 0.01))(1/3), 0.33, 1e-6);
      assert.inDelta(x.range(_.range(0, 1.1, 0.1))(1/3), 0.3, 1e-6);
      assert.inDelta(x.range(_.range(0, 1.2, 0.2))(1/3), 0.4, 1e-6);
      assert.inDelta(x.range(_.range(0, 1.25, 0.25))(1/3), 0.25, 1e-6);
      assert.inDelta(x.range(_.range(0, 1.5, 0.5))(1/3), 0.5, 1e-6);
    ...
    "invertExtent": {
      "maps a value in the range to a domain extent": function(quantize) {
        ...
        assert.deepEqual(x.invertExtent(0), [0, 0.25]);
        assert.deepEqual(x.invertExtent(1), [0.25, 0.5]);
        assert.deepEqual(x.invertExtent(2), [0.5, 0.75]);
        assert.deepEqual(x.invertExtent(3), [0.75, 1]);
      },
      "allows arbitrary range values": function(quantize) {
        var a = {}, b = {}, x = quantize().range([a, b]);
        assert.deepEqual(x.invertExtent(a), [0, 0.5]);
        assert.deepEqual(x.invertExtent(b), [0.5, 1]);
      },
