  ...
  "linear": {
    ...
    "domainSegmented": {
      "behaves like domain when n=2": function(d3) {
        var r = [5, 10], d = [0, 5];
        var s1 = d3.scale.linear().range(r), s2 = d3.scale.linear().range(r);
        s1.domain(d);
        s2.domainSegmented(d, 2);
        for (var i=0; i<17; ++i) // prime number to avoid aliasing over segments
          assert.inDelta(s1(i/17), s2(i/17), 1e-6);
      }, "segments domain uniformly in linear scales": function(d3) {
        var s1 = d3.scale.linear(), s2 = d3.scale.linear();
        s2.domainSegmented([0, 1], 5).range([0, 0.25, 0.5, 0.75, 1.0]);
        for (var i=0; i<17; ++i) // prime number to avoid aliasing over segments
          assert.inDelta(s1(i/17), s2(i/17), 1e-6);
      }, "can be called in non-numeric scales": function(d3) {
        var s1 = d3.scale.linear();
    ...
    "range": {
      ...
      "can specify range values as colors": function(d3) {
        ...
        assert.equal(x(0.5), "#800080");
        var x = d3.scale.linear().range([d3.rgb(255, 0, 0), d3.hsl(240, 1, 0.5)]);
        assert.equal(x(0.5), "#800080");
    ...
    "invert": {
      "maps a number to a number": function(d3) {
        ...
        assert.inDelta(x.invert(1.5), 0.5, 1e-6);
    ...
    "ticks": {
      ...
      "formats ticks with the appropriate precision": function(d3) {
        var x = d3.scale.linear().domain([0.123456789, 1.23456789]);
    ...
    "nice": {
      "nices the domain, extending it to round numbers": function(d3) {
        ...
        var x = d3.scale.linear().domain([0.7, 11.001]).nice();
      ...
      "has no effect on degenerate domains": function(d3) {
        ...
        var x = d3.scale.linear().domain([0.5, 0.5]).nice();
